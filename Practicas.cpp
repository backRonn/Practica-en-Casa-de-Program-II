...
//---------------------------------------------------------------------------
/*
 procedimento p·ra eliminar la primera palabra
 de una cadena.
		Ej: x = 'Hola a todos'-> x='a todos'
*/
void DelPriPal(AnsiString &x)
{
 AnsiString Pal;
 byte p,n;
 x = x.Trim();
...
void __fastcall TForm3::EliminarPrimPal1Click(TObject *Sender)
{
 AnsiString z;
 z = Edit1->Text;
 DelPriPal(z);
...
Cargar un vector
*/
void Cargar_Vector(TStringGrid *v, unsigned int dim, unsigned int pos)
{
 if (pos == dim) {
	 //Caso Base
 }else{
	 v->Cells[pos][0] = pos + 1;
	 Cargar_Vector(v,dim,pos + 1);
 }
}
void __fastcall TForm3::CargarVector1Click(TObject *Sender)
{
 Cargar_Vector(StringGrid1,StringGrid1->ColCount,0);
}
//---------------------------------------------------------------------------
/*
Escriba un proceso para eliminar el dato x
de un vector
Ej: v[2,5,65,23,45,2,13,45,61]x=2 v[5,65,23,45,13,45,61]
*/
void Eliminar_Dato(TStringGrid *v,Word n, byte x)
{
 if (n == 1) {
	 if (x == v->Cells[n][0]) {
